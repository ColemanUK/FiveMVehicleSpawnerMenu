...
local permissionsComplete = false
...
function GetPermissions()
    ...
    TriggerServerEvent("permissions.RegisterComplete")
end

RegisterNetEvent("discordMember.returnIsAllowed")
AddEventHandler("discordMember.returnIsAllowed", function(isAllowed, complete)
...
function DiscordMemberVehicleMenu(menu)
    if discordMemberVehiclesAllowed then
        ...
        subMenu.OnItemSelect = function(Sender, Item, Index)
            print(Index)
            if Item == subMenuVehicleModels[Index] and discordMemberVehiclesAllowed then
...
RegisterCommand("forcesvauth", function(source, args, rawCommand)
    GetPermissions()
    print(discordMemberVehiclesAllowed)
end, false)

RegisterCommand("forcerefresh", function(source, args, rawCommand)
    DiscordMemberVehicleMenu(menu)
    WhitelistedMemberVehicleMenu(menu)
    ServerBoosterVehicleMenu(menu)
    Tier1VehicleMenu(menu)
...
function SpawnCar(vehicleName, vehicleSpawncode)
    ...
    if not IsModelInCdimage(vehicleSpawncode) or not IsModelAVehicle(vehicleSpawncode) then
        TriggerEvent('chat:addMessage', {
            args = {'^1Perpetual Roleplay',
                    '^3Vehicle not found. Please contact server development with the following information: ^5Name - ' ..
                vehicleName}
        })
        return
    end
